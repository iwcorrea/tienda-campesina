...
async def crear_oferta_transporte(
    ...
    transporte = Transport(
        pedido_id=pedido_id,
        transportista_id=current_user["email"],
        costo=costo,
    ...
    change_transport_state(db, transporte, TransportState.PENDING_ACCEPTANCE.value,
                           changed_by=current_user["email"],
                           extra_data={"evento": "creación_oferta"})
    publicar_evento_transporte(db, "transport_offer_created",
...
async def aceptar_oferta(
    ...
    try:
        change_transport_state(db, transporte, TransportState.ACCEPTED.value,
                               changed_by=current_user["email"],
                               extra_data={"motivo": "Oferta aceptada"})
        publicar_evento_transporte(db, "transport_accepted",
                                   transport_id=transporte.id,
...
async def recoger(
    ...
    transporte = db.query(Transport).filter(Transport.id == transport_id,
                                            Transport.transportista_id == current_user["email"]).first()
    if not transporte:
        raise HTTPException(status_code=404, detail="No encontrado o no autorizado")
    try:
        change_transport_state(db, transporte, TransportState.PICKED_UP.value,
                               changed_by=current_user["email"])
        publicar_evento_transporte(db, "transport_picked_up",
                                   transport_id=transport_id,
                                   pedido_id=transporte.pedido_id,
                                   usuario_email=current_user["email"],
                                   estado_anterior=TransportState.ACCEPTED.value,
                                   estado_nuevo=TransportState.PICKED_UP.value,
                                   descripcion="Recogida confirmada")
        db.commit()
        db.refresh(transporte)
        return transporte
    except ValueError as e:
...
async def entregar(
    ...
    if not transporte:
        raise HTTPException(status_code=404, detail="No encontrado o no autorizado")
    try:
        change_transport_state(db, transporte, TransportState.DELIVERED.value,
                               changed_by=current_user["email"])
        publicar_evento_transporte(db, "transport_delivered",
                                   transport_id=transport_id,
                                   pedido_id=transporte.pedido_id,
                                   usuario_email=current_user["email"],
                                   estado_anterior=TransportState.IN_TRANSIT.value,
                                   estado_nuevo=TransportState.DELIVERED.value,
