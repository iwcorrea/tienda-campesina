...
def inicializar_stock(db: Session, producto: Producto, cantidad: int, referencia: str = "creacion") -> Optional[MovimientoInventario]:
    existe = db.query(MovimientoInventario).filter(MovimientoInventario.producto_id == producto.id).first()
    if existe:
        return None
