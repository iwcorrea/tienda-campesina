...
from app.modules.orders.payment import (
    crear_pago,
    confirmar_pago,
...
def checkout(request: Request, pedido_id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if not current_user:
    ...
    return templates.TemplateResponse("pago_checkout.html", {
        "request": request, "pedido": pedido, "total": total,
        "comision_plataforma": monto_comision, "monto_vendedor": monto_vendedor,
        "porcentaje_comision": COMISION_PLATAFORMA, "costo_envio": costo_envio,
    })

@router.post("/iniciar/{pedido_id}")
def iniciar_pago(request: Request, pedido_id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if not current_user:
        return RedirectResponse(url="/auth/login", status_code=303)
...
def procesar_pago(request: Request, pago_id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if not current_user:
        return RedirectResponse(url="/auth/login", status_code=303)
    ...
    return templates.TemplateResponse("pago_procesar.html", {"request": request, "pago": pago_vm})

@router.post("/confirmar/{pago_id}")
def confirmar_pago_post(request: Request, pago_id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if not current_user:
        return RedirectResponse(url="/auth/login", status_code=303)
...
def pago_exitoso(request: Request, pago_id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if not current_user:
        return RedirectResponse(url="/auth/login", status_code=303)
    pago = db.query(Pago).filter(Pago.id == pago_id).first()
    if not pago:
        return RedirectResponse(url="/pedidos", status_code=303)
    pago_vm = PagoViewModel.from_orm(pago)
    return templates.TemplateResponse("pago_exito.html", {"request": request, "pago": pago_vm})
